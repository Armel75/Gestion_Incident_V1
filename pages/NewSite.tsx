...
export const NewSite: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // getTypes est paginé
        const list = await api.getTypes(1, 200);
        setTypes(list);

        if (isEditMode && id) {
          const site = await api.getSiteById(id);
          if (site) {
            setFormData({
              name: site.name ?? '',
              // ✅ string obligatoire pour select
              typeId: site.typeId != null
                ? String(site.typeId)
                : site.type?.id != null
                  ? String(site.type.id)
                  : ''
          ...
          // ✅ en création : pré-sélection simple du premier type
          setFormData(prev => ({
            ...prev,
            typeId: prev.typeId || (list[0]?.id != null ? String(list[0].id) : '')
          }));
        }
      } catch (e) {
        console.error(e);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const typeIdNum = Number(formData.typeId);
    if (!Number.isInteger(typeIdNum) || typeIdNum <= 0) {
      return; // afficher une erreur UI si tu veux
    }
    ...
    try {
      ...
      if (isEditMode && id) {
        await api.updateSite(id, payload as any);
      } else {
        await api.createSite(payload as any);
      }
      ...
      <div className="flex-1 overflow-auto p-6 lg:p-10 max-w-2xl mx-auto w-full">
        <section className="bg-white dark:bg-slate-900 rounded-lg shadow-xs border border-slate-200 dark:border-slate-800 p-6">
          <div className="space-y-6">
            {/* Type */}
            <div>
              <label htmlFor="typeId" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
