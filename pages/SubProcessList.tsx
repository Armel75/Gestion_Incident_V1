...
import { SubProcess, Process } from '../types';
import { useNavigate } from 'react-router-dom';
import { ArrowLeft, Edit2, Trash2, Plus } from 'lucide-react';

export const SubProcessList: React.FC = () => {
  const [items, setItems] = useState<SubProcess[]>([]);
  ...
  const handleDelete = async (id: string, name: string) => {
      if (window.confirm(`Êtes-vous sûr de vouloir supprimer le sous-processus "${name}" ?`)) {
          await api.deleteSubProcess(id);
          const data = await api.getSubProcesses();
          setItems(data);
      }
  };
    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 transition-colors duration-200">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 sticky top-0 z-20 transition-colors duration-200">
         <div className="flex items-center gap-4">
            ...
            <h1 className="text-lg font-semibold text-slate-900 dark:text-white">Gestion des Sous-processus</h1>
         </div>
         <div>
             <button 
                onClick={() => navigate('/settings/sub-processes/new')}
             ...
             >
                 <Plus className="h-4 w-4" /> Nouveau sous-processus
             </button>
         </div>
      </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm max-w-4xl mx-auto">
                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    <thead className="bg-slate-50 dark:bg-slate-950">
                        <tr>
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Processus</th>
                                ...
                                <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex items-center justify-end gap-3">
                                            <button 
                                                onClick={() => navigate(`/settings/sub-processes/${item.id}/edit`)}
