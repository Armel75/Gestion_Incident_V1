...
export const ProcessList: React.FC = () => {
  ...
  const handleDelete = async (id: string, name: string) => {
      if (window.confirm(`Êtes-vous sûr de vouloir supprimer le processus "${name}" ?`)) {
    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 transition-colors duration-200">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 sticky top-0 z-20 transition-colors duration-200">
         <div className="flex items-center gap-4">
            ...
            <h1 className="text-lg font-semibold text-slate-900 dark:text-white">Gestion des Processus</h1>
         </div>
         <div>
             <button 
                onClick={() => navigate('/settings/processes/new')}
                                ...
                                <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex items-center justify-end gap-3">
                                            <button 
                                                onClick={() => navigate(`/settings/processes/${item.id}/edit`)}
